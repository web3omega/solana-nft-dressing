...
export const Trait: React.FC<{ nft: Nft, onClick: any, apply: boolean }> = ({ nft, onClick, apply }) => {
  return <div className='border m-2 p-2 flex rounded border-gray-800 bg-zinc-800 drop-shadow-lg h-56'>
    ...
    <button className="p-1 absolute bottom-1 border left-1/2 -translate-x-1/2 rounded" onClick={() => onClick()}>{apply ? 'Enable Trait' : 'Remove Trait'}</button>
  </div>
}